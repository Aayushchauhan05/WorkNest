...
const {
  ...
  Totalprojectlistedbybusiness,
} = require("../models/Business/ProjectSchema");
...
const ListprojectBybusiness = async (req, res) => {
  try {
    ...
    const project = await ProjectListByBusiness.create({
      ...frontenddata,
      Email: Email,
    });
    const { _id } = project;
...
const getprojectdata = async (req, res) => {
  try {
    const { Email } = req.user;
    const data = await Business.findOne({ Email }).populate("ProjectList");
...
const project_reg = async (req, res) => {
  try {
    ...
    // console.log(._id);
    const project = await Freelancer.findByIdAndUpdate(
      id,
      { $push: { project: _id } },
      { new: true }
    );
...
const deleteProjectFreelancer = async (req, res) => {
  try {
    ...
    if (!projectExist) {
      return res
        .status(404)
        .json({ message: "No project found with this name" });
    }
